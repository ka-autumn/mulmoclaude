...
- preset の判定は **「アクティブにある」かつ「カタログ preset にも同じ slug がある」** にすべきか、それとも **slug が `mc-` 接頭辞** で判定すべきか。今回は後者（slug 接頭辞）を採用。理由: カタログのロードを待たずに即時判定できる / 接頭辞は `isPresetSlug()` で既に launcher 側でも使われている命名規約。前者にすべきなら catalogPresets の load 完了待ちが必要
...
```
アクティブ: 毎ターン自動でプロンプトに読み込まれるスキル（lock = mc- 同梱 / folder = プロジェクト / home = ユーザー共通）。
カタログ: 使えるけどまだ有効化していないスキル。プロンプトには含まれません。★スターでアクティブに移動（以降ずっと有効）、▶今だけ実行は 1 回だけ呼び出し（永続化しない）。
```
...
- `isSelectedPreset` computed を追加 — `detail.value?.name.startsWith("mc-")` で判定
